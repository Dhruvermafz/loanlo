...
router.post("/new/loan", auth, async (req, res) => {
  try {
    const { payableAmount, interest } = calcAmount(
      parseInt(req.body.amount),
      req.body.emiDuration / 12
    );
    const date = new Date(req.body.startDate);
    const expiryDate = new Date(
      date.setMonth(date.getMonth() + req.body.emiDuration / 12)
    );
    const loan = new Loan({
      owner: req.id,
      ...req.body,
...
router.get("/loans", auth, async (req, res) => {
  try {
    const user = await User.findById(req.id).populate("loans");
